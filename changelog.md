...
- **Army storage and searchable memory** — Storage conventions (§5a) in OPENCLAW_ARMY_OF_OPENCLAW.md: canonical memory keys (`node:<id>:lessons`, `lessons_by_role:<role>`, `lessons_daily:YYYY-MM-DD`, `army.order.<orderId>`, `intel:<topic>`). Full-text search (SQLite FTS5): mesh/store schema and client add `mesh_memory_fts` table; `putMemory` syncs FTS; `searchMemory(query, { scope?, limit? })` returns `{ scope, key, snippet }[]`. Store API: `GET /mesh/memory?q=` and `POST /mesh/query` for search. FTS backfill on open for existing DBs; `runFtsBackfill(dbPath)` export for manual run. OPENCLAW_ARMY_SOUL_BY_RANK.md: search_memory instruction in §1.1 Learning and General Tools. army/README.md and mesh/store/README.md: search usage. docs/SEMANTIC_MEMORY_DESIGN.md: v1 FTS implemented, v2 vector design-only.

- **Vector (Phase 2) semantic search** — mesh/store/embedding.js: Ollama `/api/embed` and OpenAI-compatible embedding API. sqlite-vec: `mesh_memory_vec` vec0 table, cosine distance. `putMemory` optionally syncs vectors when `MESH_VECTOR_ENABLED=1` and `MESH_EMBEDDING_URL` set. `semanticSearchMemory(query, opts)` async returns `{ scope, key, distance }[]`. Store API: `GET /mesh/memory?q=&mode=semantic` and `POST /mesh/query` with `mode: 'semantic'`. scripts/vec-backfill.js and `npm run run:vec-backfill` for existing memories. Env: MESH_EMBEDDING_MODEL, MESH_EMBEDDING_DIMENSIONS.
